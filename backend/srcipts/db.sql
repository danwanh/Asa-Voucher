...
CREATE TABLE public.orders (
  ...
  payment_method character varying NOT NULL CHECK (payment_method::text = ANY (ARRAY['vnpay'::character varying, 'paypal'::character varying]::text[])) NOT VALI),
...
CREATE TABLE public.payments (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  order_id uuid NOT NULL,
  method character varying NOT NULL CHECK (method::text = ANY (ARRAY['vnpay'::character varying, 'paypal'::character varying]::text[])) NOT VALI),
  amount numeric NOT NULL,
  status character varying NOT NULL DEFAULT 'pending'::character varying CHECK (status::text = ANY (ARRAY['pending'::character varying, 'processing'::character varying, 'success'::character varying, 'failed'::character varying, 'refunded'::character varying]::text[])) NOT VALI),
